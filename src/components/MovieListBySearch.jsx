...
const MovieListBySearch = ({searchQuery}) =>{
    ...
    return(
        movieData.map((movie)=>(
            <article data-movie="Movie Title One Action 2026" className="overflow-hidden rounded-[18px] border border-white/10 bg-[#101521] transition hover:-translate-y-1 hover:border-white/20 relative" key={movie.show.id}>
                <div className="absolute left-3 top-3 rounded-full g-black/75 px-2.5 py-1 text-base font-bold backdrop-blur-sm">
                                ⭐  <span>•</span> {movie.show.rating.average} 
                </div>
                ...
                <div className="p-4">
                    <h2 className="text-xl font-semibold">{movie.show.name}</h2>
                    <span className="text-base text-slate-500">📅 <span>•</span> {movie.show.premiered}</span>
                </div>
                <div className="p-4">
                    <button
                    onClick={()=>setMovieId(movie.show.id)}
